...
def parse_args():
    ...
    parser.add_argument(
        "--profile", action="store_true", help="Profile the run using pyinstrument"
    )
...
def save_results(path, outputs):
    ...
    for out in outputs:
        out_cpu = {
            k: v.clone().detach().to("cpu").numpy()
            if isinstance(v, torch_tensor_cls) and v.requires_grad
            else v.clone().to("cpu").numpy()
            if isinstance(v, torch_tensor_cls)
            else v
