...
class RandomRewardModel(BaseRewardModel):
    def __init__(
        self, seed=0, distribution="uniform", p0=1, p1=0, metagraph=None, **kwargs
    ):
        super(RandomRewardModel, self).__init__(metagraph=metagraph)
        self.forward_value = RandomValue(
            seed=seed, distribution=distribution, p0=p0, p1=p1
        )
        self.backward_value = RandomValue(
            seed=seed,
            distribution=distribution,
            p0=kwargs.get("backward_p0", p0),
...
class CustomRewardModel(RewardModel):
    def __init__(
        self, model_path: str, device: str, config: "bittensor.config" = None, **kwargs
    ):
