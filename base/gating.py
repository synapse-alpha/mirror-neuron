...
class BaseGatingModel(torch.nn.Module, ABC):
    ...
    def get_gradients(model):
        """ Grab the gratiends for the current step.
            Args:
                model (:obj:`torch.nn.Module`) : The model to get the gradients from.
            Returns:
                step_grads (:obj:`dict` of :obj:`torch.FloatTensor`): Dictionary of gradients for each parameter in the model.
        """
        step_grads = {}
...
class SequentialGatingModel(BaseGatingModel):
    def __init__(
        ...
        self.model = torch.nn.Sequential(*([self.embedding_layer] + self.layers)).to(
            self.device
        )
        # generalize this so that num_hidden can be zero/empty so that we just learn weights from embedding space to uid space

        # Linear layer
        self.linear = torch.nn.Linear(self.hidden_size[-1], self.metagraph.n).to(
            self.device
        )
        ...
        self.gradient_history = queue.Queue(maxsize=self.config.neuron.max_history)
        self.optimizer_history: list = []
        self.save_gradient_steps = 1
        self.save_gradient_path = 'gradients'
        self.register_hook()

    # TODO: Make this nice, maybe save as a list before saving to disk at end??
    def register_hook(self):
        # Create a hook to store gradients every N steps
        os.makedirs(self.save_gradient_path, exist_ok=True)
        def save_grad(name):
            step = 0
            def hook(grad):
                ...
                nonlocal step
                step += 1
                if step % self.save_gradient_steps == 0:
                    np.save(os.path.join(self.save_gradient_path, f'{name}_step{step}.npy'), grad.detach().cpu().numpy())
            return hook
        # Register the hook for each parameter in your model
    ...
    def backward(self, scores: torch.FloatTensor, rewards: torch.FloatTensor):
        ...
        # Zero gradients before computing backward pass
        self.optimizer.zero_grad()

        # Backward pass
        loss.backward()

        # Attempt to record gradients for each step
        self.history.put(SimpleNamespace(loss=loss.item()))
        self.gradient_history.put(SimpleNamespace(**self.get_gradients(self.linear)))

        # Capture optimizer state_dict in hopes gradients show up there
        self.optimizer.step()
        self.optimizer_history.append(self.optimizer.state_dict())
...
class HuggingFaceGatingModel(BaseGatingModel):
    ...
    def backward(self, scores: torch.FloatTensor, rewards: torch.FloatTensor):
        ...
        loss.backward()
        self.history.put(SimpleNamespace(loss=loss.item()))
        self.gradients.put(SimpleNamespace(**self.get_gradients(self.linear)))
        self.optimizer.step()
