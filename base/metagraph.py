...
class DummyMetagraph( bittensor.metagraph ):
    ...
    def __init__(self, netuid: int, network: str = 'finney', lite:bool = True, sync: bool = True, num_nodes = 16 ) -> 'metagraph':    
    ...
    def sync ( self, block: Optional[int] = 0, lite: bool = True ) -> 'metagraph':
        ...
        for i in range(1,self.num_nodes+1):
            ...
            neuron.uid = i
            neuron.axon_info = DummyAxonInfo(hotkey=neuron.hotkey, coldkey=neuron.coldkey )
            self.neurons.append(neuron)
            
        self.lite = lite
        self.n = torch.nn.Parameter( torch.tensor( len(self.neurons), dtype=torch.int64 ), requires_grad=False )
        self.version = torch.nn.Parameter( torch.tensor( [bittensor.__version_as_int__], dtype=torch.int64 ), requires_grad=False )
        self.block = torch.nn.Parameter( torch.tensor( block, dtype=torch.int64 ), requires_grad=False )
